...
@TODO make a sign-up sheet so folks can pick what they want to do

We picked out three specific goals the project has which we think will make simple yet very helpful contributions today:

1. Updating dependencies to use `^` ranges (link issue and repos)
2. Adding engines fields to packages (link issue and repos)
3. Documentation updates for v5 (link repo)

And we also picked out a few issues which would be great ones to try to triage:
